...
(* ###############################################################
   Helios version 4 with homomorphic tally and pseudonyms,
   obtain from HeliosAbstract with 
   -1. the label (Flabel)   = const
...
clone export Helios_hom as Hv4H with
  type HH.label      <- label,
  type ident         <- ident,
  type HH.skey       <- skey,
  type HH.pkey       <- pkey,
...
(* ---------------------------------------------------------------------- *)
(* Helios Version 4 with Homomorphic tally and alias *) 
module Heliosv4H (Pe: PSpke.Prover, Ve: PSpke.Verifier, 
                  Ve2: PSpke.Verifier,
...
equiv Hv4H2_MV_tally &m:
  MV(IPKE(Pe,Ve),Pz,Vz, ValidProof(Ve2), HRO.RO, G).tally ~
  Heliosv4H(Pe, Ve, Ve2, Pz, Vz, HRO.RO, G).tally  
...
local lemma scorr_bpriv (LR<: LorR { BS, BP, BIND, BIND2, BSCorr, BSC, 
  ...
  Pr[SCorr (Heliosv4H(Pe, Ve, Cv, Pz, Vz), 
            BSCorr(Heliosv4H(Pe, Ve, Cv, Pz, Vz),A, LR), 
...
proof. 
  have ->: Pr[SCorr (Heliosv4H(Pe, Ve, Cv, Pz, Vz), 
                     BSCorr(Heliosv4H(Pe, Ve, Cv, Pz, Vz),A, LR), 
                     HRO.RO, S).main() @ &m: BSC.valid] =
...
lemma bpriv &m: 
   `|Pr[BPRIV_L(Heliosv4H(Pe, Ve, Cv, Pz, Vz), A, HRO.RO, G).main() @ &m : res] -
     Pr[BPRIV_R(Heliosv4H(Pe, Ve, Cv, Pz, Vz), A, HRO.RO, G, S).main() @ &m :res]|
<=
...
proof.
  have ->: Pr[BPRIV_L(Heliosv4H(Pe, Ve, Cv, Pz, Vz), A, HRO.RO, G).main() @ &m : res] =
           Pr[BPRIV_L(HeliosHom(Pe, Ve, Pz, Vz, ValidProof(Cv)), A, HRO.RO, G).main() @ &m : res].
    by byequiv =>//=; sim.
  have ->: Pr[BPRIV_R(Heliosv4H(Pe, Ve, Cv, Pz, Vz), A, HRO.RO, G, S).main() @ &m : res] =
  ...
  have ->: Pr[SCorr(MV(IPKE(Pe, Ve), Pz, Vz, ValidProof(Cv)), 
              ...
              HRO.RO, S).main() @ &m : BSC.valid] = 
           Pr[SCorr (Heliosv4H(Pe, Ve, Cv, Pz, Vz), 
              BSCorr(Heliosv4H(Pe, Ve, Cv, Pz, Vz),A, Left), 
              HRO.RO, S).main() @ &m: BSC.valid].
    by byequiv =>//=; sim.
  ...
  have ->: Pr[SCorr(MV(IPKE(Pe, Ve), Pz, Vz, ValidProof(Cv)), 
              ...
              HRO.RO, S).main() @ &m : BSC.valid] = 
           Pr[SCorr (Heliosv4H(Pe, Ve, Cv, Pz, Vz), 
              BSCorr(Heliosv4H(Pe, Ve, Cv, Pz, Vz),A, Right), 
              HRO.RO, S).main() @ &m: BSC.valid].
    by byequiv =>//=; sim.
...
lemma consis1(id: ident, v: plain, l: label) &m: 
   Pr[SConsis1(Heliosv4H(Pe, Ve, Cv, Pz, Vz), CE(IPKE(Pe,Ve)), HRO.RO, G).main(id,v, l) @ &m: res]  >=
   Pr[PKEvf.Correctness(IPKE(Pe,Ve), HRO.RO).main(v,l) @ &m: res].
proof.
  have ->: Pr[SConsis1(Heliosv4H(Pe, Ve, Cv, Pz, Vz), 
...
lemma consis2 &m:
  Pr[SConsis2(Heliosv4H(Pe, Ve, Cv, Pz, Vz), ValidProof(Cv), AC2, HRO.RO, G).main() @ &m: res] = 1%r.
proof. 
  have ->: Pr[SConsis2(Heliosv4H(Pe, Ve, Cv, Pz, Vz), 
...
equiv consis3 &m:
    SConsis3_L(Heliosv4H(Pe, Ve, Cv, Pz, Vz), ValidProof(Cv), AC3, HRO.RO, G).main ~ 
    SConsis3_R(Heliosv4H(Pe, Ve, Cv, Pz, Vz), CE(IPKE(Pe,Ve)), ValidProof(Cv), AC3, HRO.RO, G).main
...
lemma scorr (AS <: SCorr_Adv' { IPKE(Pe,Ve), Pz, Vz, Cv, HRO.RO, BSC, BS, G}) &m:
  Pr[SCorr(Heliosv4H(Pe, Ve, Cv, Pz, Vz), 
           AS(Heliosv4H(Pe, Ve, Cv, Pz, Vz)), HRO.RO, G).main() @ &m: BSC.valid] = 
`|Pr[Ind1CCA(IPKE(Pe,Ve), 
             INDB(Bmem(MV(IPKE(Pe,Ve), Pz, Vz, ValidProof(Cv)), 
