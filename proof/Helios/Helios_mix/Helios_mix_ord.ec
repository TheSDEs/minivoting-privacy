...
require import Distr NewDistr Int IntExtra FMap FSet Real Pair Mu_mem. 
...
op Rho (ubb:(ident * vote option) list)  : (vote list) distr
       = if (removeInvalid ubb = []) then MUnit.dunit []
         else MUnit.dunit (sort (<=) (map snd (Policy (removeInvalid ubb)))).
...
(* lemma for distribution of permutations *)
local lemma nosmt Rho_weight (x: (ident * vote option) list):
  weight (Rho x) = 1%r. 
proof.  
  cut t:= MUnit.dunit_ll (sort (<=) (map snd (Policy (removeInvalid x)))).
